...
{
    public class ProcessActor : TracedActor
    {
        ...
        protected override void OnReceive(object messageArg)
        {
            switch (messageArg)
            {
                case ProcessRequest message:
                    ...
                    Sender.Tell(new ProcessAnswer($"{message.Content}_processed"));
                    break;
        ...
        protected void Register(Member member)
        {
            ...
            {
                Context.ActorSelection(member.Address + "/user/frontend").Tell(new RegisterBackEnd());
            }                
        }
    }
}
